...
In the app we've just created, we store all contacts in a single plaintext field, like skype, telephones, emails etc. Now let us make it so that each customer may have several separate contact records. We also  want contacts of each customer to show on a separate tab.
