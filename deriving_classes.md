...
4. In the class *CmsCustomer*, we already have the string-type property *interaction*, but we want it to be link-type. 
   ...
   So, delete the property *interaction* and create a new property with similar name. Set its parameters `Type: LINKLIST`, `Visualization: table`, `Linked Class: CmsInteraction`, `Inverse: customer`.
